...
export default function Main(props) {
  ...
  const [clickMoveStr, setClickMoveStr] =
    React.useState("");
  const [withinTurns, setWithinTurns] = React.useState();
  const [isStalemate, setIsStalemate] =
    React.useState(false);
  const [checkmatedColor, setCheckmatedColor] =
    React.useState(null);
  const [checkedColor, setCheckedColor] =
    React.useState(null);
  ...
  function boardRow(rowArr, rowIdx) {
    return (
      ...
      >
        <Typography className="boardNumber">
          {boardNums[rowIdx]}
        </Typography>
  ...
  function boardLocation(spaceOrPiece, rowIdx, spaceIdx) {
    ...
    const position = boardPositionToString(
      rowIdx,
      spaceIdx
    );
  ...
  async function getMoveResponse() {
    ...
    const [moveStartX, moveStartY] =
      stringToBoardPosition(moveStrStart);
    const [moveEndX, moveEndY] =
      stringToBoardPosition(moveStrEnd);
    const newBoard = board.copyBoard();
    // Save piece from start pos
    const savedPieceColor =
      newBoard.board[moveStartX][moveStartY].color;
    const savedPieceType =
      newBoard.board[moveStartX][moveStartY].pieceType;
      ...
      <Container>
        ...
        <Paper elevation={3} className="boardContainer">
          {board ? (
            ...
            <Typography>
              No board found. Please reload the page and try
              again.
            </Typography>
          )}
          {board ? (
            <>
              <Stack
                ...
                justifyContent="center"
              >
                <Typography className="boardLetters">
                  {boardLetters.map((letter) => letter)}
                </Typography>
              </Stack>
              <BoardStatusText
                board={board}
                ...
                currTurn={currTurn}
                withinTurns={withinTurns}
              />
              <EnterMoves
                isStalemate={isStalemate}
                checkmatedColor={checkmatedColor}
                clickMoveStr={clickMoveStr}
